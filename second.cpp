...
main( int argc, char *argv[ ] )
{
	...
	float *hD = new float[ NUM_ELEMENTS ]; // Add a fourth array for **PART 2**.

	// fill the host memory buffers:

	for( int i = 0; i < NUM_ELEMENTS; i++ )
	{
		hA[i] = hB[i] = hC[i] = (float) sqrt(  (double)i  ); // Add hC[i] for **PART 2**.
	}
	...
	cl_mem dC = clCreateBuffer( context, CL_MEM_READ_ONLY, dataSize, NULL, &status ); //**PART2** change to READ ONLY
	if( status != CL_SUCCESS )
		fprintf( stderr, "clCreateBuffer failed (3)\n" );
		
	cl_mem dD = clCreateBuffer( context, CL_MEM_WRITE_ONLY, dataSize, NULL, &status ); // **PART2**
		if( status != CL_SUCCESS )
	fprintf( stderr, "clCreateBuffer failed (4)\n" );

	// 6. enqueue the 2 (3 **PART2**) commands to write the data from the host buffers to the device buffers:
	...
	status = clEnqueueWriteBuffer( cmdQueue, dC, CL_FALSE, 0, dataSize, hC, 0, NULL, NULL ); // **PART2**
	if( status != CL_SUCCESS )
	...
	status = clSetKernelArg( kernel, 3, sizeof(cl_mem), &dD ); // **PART2**
	if( status != CL_SUCCESS )
	...
	size_t globalWorkSize[4] = { NUM_ELEMENTS, 1, 1 }; // **PART2** increased size to 4
	size_t localWorkSize[4]  = { LOCAL_SIZE,   1, 1 }; // **PART2** increased size to 4
	...
	status = clEnqueueReadBuffer( cmdQueue, dD, CL_TRUE, 0, dataSize, hD, 0, NULL, NULL ); //**PART2** changed to D
	if( status != CL_SUCCESS )
